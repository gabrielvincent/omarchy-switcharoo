...
function normalizedConfig(entry, payload) {
    ...
    var direction = String(
        pick("direction", "initialDirection", "next") || "next",
    ).toLowerCase();
    ...
    var switchMode = String(
        pick("switchMode", "switch_mode", "clients") || "clients",
    ).toLowerCase();
    switchMode =
        switchMode === "workspace" || switchMode === "workspaces"
            ? "workspaces"
            : "clients";

    var excludeWorkspaces = normalizeWorkspacePatterns(
        pick("excludeWorkspaces", "exclude_workspaces", []),
    );

    return {
        switchMode: switchMode,
        excludeWorkspaces: excludeWorkspaces,
        itemsPerRow: clampInteger(
            pick("itemsPerRow", "maxItemsPerRow", 4),
            4,
            1,
            12,
        ),
        maxVisibleRows: clampInteger(
            pick("maxVisibleRows", "maxRows", 3),
            3,
            1,
            10,
        ),
...
function hasFilter(config, name) {
    var filters =
        config && Array.isArray(config.filterBy) ? config.filterBy : [];
...
function isExcludedWorkspace(workspace, config) {
    var patterns =
        config && Array.isArray(config.excludeWorkspaces)
    ...
    for (var i = 0; i < patterns.length; i++) {
        try {
            ...
            // Ignore invalid user patterns rather than breaking the switcher.
        }
    }
    return false;
}
...
function filteredClients(snapshot, config) {
    ...
    for (var j = 0; j < source.length; j++) {
        ...
        if (isExcludedWorkspace(client.workspace, config)) continue;

        if (
            active &&
            hasFilter(config, "current_monitor") &&
            Number(client.monitor) !== Number(active.monitor)
        )
            continue;
        if (active && hasFilter(config, "current_workspace")) {
            var workspaceId = client.workspace
                ? Number(client.workspace.id)
                : NaN;
            var activeWorkspaceId = active.workspace
                ? Number(active.workspace.id)
                : NaN;
            if (workspaceId !== activeWorkspaceId) continue;
        }
        if (
            active &&
            hasFilter(config, "same_class") &&
            String(client.class || "").toLowerCase() !==
                String(active.class || "").toLowerCase()
        )
            continue;
...
function filteredWorkspaces(snapshot, config, activeWorkspaceId, clients) {
    ...
    for (var c = 0; c < clientSource.length; c++) {
        ...
        if (existing === undefined || rank < existing)
            recencyById[clientWorkspaceId] = rank;
    }
    ...
    for (var j = 0; j < source.length; j++) {
        ...
        var name = String(workspace.name || "");
        if (!isFinite(id) || !name || isExcludedWorkspace(workspace, config))
            continue;
        if (
            hasFilter(config, "current_monitor") &&
            isFinite(activeMonitorId) &&
